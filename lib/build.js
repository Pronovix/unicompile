...
const buildSass = async (settings) => {
  try {
    ...
    const outDir = pathJoin(localSettings.out, 'css');
...
const buildJs = async (settings) => {
  try {
    ...
    const outDir = pathJoin(localSettings.out, 'js');
...
const build = async (args) => {
  try {
    const promises = [];
    const settings = {};
    settings['output-style'] = args.outputStyle;
    ...
    if (config === false) {
      ...
      if (args.out) {
        settings.out = pathNormalize(args.out);
      } else {
        settings.out = pathJoin(settings.cwd, 'dist');
      }
      ...
      await config.map(async (component) => {
        ...
        if (component.out && component.out.length > 0) {
          settings.out = pathNormalize(component.out);
        }

        // If there was no argument given and output wasn't defined in the component, fall back to default.
        if (!args.out && !component.out) {
          settings.out = pathJoin(settings.cwd, 'dist');
        }
