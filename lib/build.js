  ...
  try {
    ...
    await Promise.all(
      files.map(async (file) => {
        ...
        messages.push(`${file} -> ${cyan(`${outFile}`)}`);
        if (args.s) {
          await fsPromises.writeFile(`${outFile}.map`, result.map.toString());
          messages.push(`${file} -> ${cyan(`${outFile}.map`)}`);
        }
      }),
    );
    messages.push(green(`Built ${args.s ? fileCount * 2 : fileCount} SASS files to ${pathResolve(outDir)}.`));
...
const watch = (args) => {
  const watchPaths = [];
  settings.map((config) => {
    const absCwd = pathResolve(config.cwd);
